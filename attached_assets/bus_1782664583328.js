...
/* ══ PRESET STOPS ═══════════════════════════════════════════ */
const PRESET_STOPS = [
  { operator:'CTB', id:'001311',           label:'富景花園',		route:'789', serviceType:1, hint:'往金鐘樂禮街總站' },
  { operator:'CTB', id:'001311', 			label:'富景花園',     route:'788', serviceType:1, hint:'往中環 (港澳碼頭)' },
  { operator:'CTB', id:'001384', 			label:'富怡花園',       route:'118', serviceType:1, hint:'往長沙灣 (深旺道) 總站' },  
  { operator:'KMB', id:'5C1471822EE10AF3', label:'富怡花園',       route:'118', serviceType:1, hint:'往長沙灣 (深旺道) 總站' },
];
...
/* ══ HOME DASHBOARD BUS ETA GENERATOR (MULTIPLE TRIPS) ══════════════════════ */

async function initHomeBusETA() {
  const container = document.getElementById('h-bus-content');
  if (!container) return;

  container.innerHTML = '<div style="color:var(--text-muted);font-size:13px;padding:10px 0;text-align:center;">正在同步實時班次...</div>';

  try {
    const promises = PRESET_STOPS.map(async (stop) => {
      let url = '';
      if (stop.operator === 'KMB') {
        url = `https://data.etabus.gov.hk/v1/transport/kmb/eta/${stop.id}/${stop.route}/${stop.serviceType || 1}`;
      } else if (stop.operator === 'CTB') {
        url = `https://rt.data.gov.hk/v2/transport/citybus/eta/CTB/${stop.id}/${stop.route}`;
      }
      ...
      try {
        ...
        // 1. 這次我們保留所有有時間數據的班次 (最多取 3 班)
        let nextTrips = [];
        if (json.data && json.data.length > 0) {
          // 根據你的 bus.js 原始邏輯：城巴需要過濾方向(一般用本地邏輯)，九巴依據 dir 
          // 這裡篩選出所有含有有效 eta 的班次
          const validTrips = json.data.filter(item => item.eta);
          
          // 最多拿前 3 班車的預計到站時間
          nextTrips = validTrips.slice(0, 3).map(item => item.eta);
        }
        return { ...stop, nextTrips: nextTrips };
      } catch (err) {
        return { ...stop, nextTrips: [], error: true };
      }
    });
    ...
    results.forEach(item => {
      ...
      // 2. 判斷是否有班次資料
      if (item.nextTrips && item.nextTrips.length > 0) {
        // 遍歷所有拿到的班次時間，生成多個時間標籤
        const tagsHTML = item.nextTrips.map((etaTime, index) => {
          const minLeft = Math.max(0, Math.floor((new Date(etaTime) - new Date()) / 1000 / 60));
          
          // 第一班車給比較醒目的樣式，第 2、3 班車給稍微淡一點的顏色
          if (minLeft <= 0) {
            return `<span class="tag tag-green" style="font-size:11px; padding:2px 6px;">即將到站</span>`;
          } else {
            const opacityStyle = index === 0 ? 'color:var(--primary); font-size:16px;' : 'color:var(--text-muted); font-size:13px; opacity:0.8;';
            const separator = index < item.nextTrips.length - 1 ? '<span style="color:var(--border); margin: 0 4px;">|</span>' : '';
            return `
              <span style="font-family:var(--font-mono); font-weight:700; ${opacityStyle}">
                ${minLeft}<span style="font-size:10px; font-weight:400; ${index === 0 ? 'color:var(--text-muted)' : ''}">分</span>
              </span>${separator}
            `;
          }
        }).join('');

        timeContainerHTML = `<div style="display:flex; align-items:center; gap:4px;">${tagsHTML}</div>`;
      } else {
        timeContainerHTML = '<span style="color:var(--text-faint); font-size:13px;">暫無班次</span>';
      }

      // 3. 建立與專案外觀 100% 吻合的列元素
      const row = document.createElement('div');
      row.className = 'row-item';
      row.style.cssText = 'display:flex; justify-content:space-between; align-items:center; padding:12px 14px;';
      row.innerHTML = `
        <div>
          <div style="display:flex; align-items:center; gap:8px;">
            <span class="tag ${item.operator === 'KMB' ? 'tag-red' : 'tag-yellow'}" style="padding:1px 6px; font-size:10px;">${item.operator}</span>
            <strong style="font-size: 22px; font-weight: 800; color: ${item.operator === 'KMB' ? '#dc2626' : '#d97706'}; font-family: var(--font-mono); line-height: 1;">
              ${item.route}
            </strong>
          </div>
          <div class="row-sub" style="margin-top:4px; font-size:12px; color:var(--text-muted);">${item.label} — ${item.hint}</div>
        </div>
    ...
    container.innerHTML = `<div style="color:var(--error);font-size:12px;text-align:center;">載入失敗: ${e.message}</div>`;
  }
}

// 自動與網頁系統綁定
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(initHomeBusETA, 300);
  setInterval(initHomeBusETA, 60000);
});

window.initHomeBusETA = initHomeBusETA;